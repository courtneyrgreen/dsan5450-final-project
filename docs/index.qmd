...
```{mermaid}
flowchart LR
  A[Race / Demographics] --> B[Police Presence t]
  C[Prior Arrests t-1] --> B
  D[Actual Crime] --> E[Reported Crime]
  D --> F[Arrests t]
  B --> E
  B --> F
  E --> F
  F --> G[Police Presence t+1]
```
...
### Recommendation 1: Audit the Data-Generating Process Before Using Any Tool
...
### Recommendation 2: Separate Officer-Initiated Data from Civilian-Reported Data in All Public Safety Reporting
...
### Recommendation 3: Establish Sunset Requirements for Arrest-Based Risk Models
